...
def read_paths_and_labels_with_doc_label(filename):
    paths = []
    labels = []
    doc_labels = []
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for idx, row in enumerate(reader, 1):
            ...
            labels.append(int(label))
    # return paths[:100] + paths[-100:], labels[:100] + labels[-100:]
    return paths, labels, doc_labels
...
class PolypDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for img_path in images:
            ...
            img_label_8class = get_class_map(self.patient_label_map, os.path.split(img_path)[-2], os.path.split(img_path)[-1], eight_class=True)
            img_labels_8class.append(torch.tensor(img_label_8class).unsqueeze(0))

            name_tensor.append(os.path.split(img_path)[-1])
        
        return torch.stack(images_tensor), torch.tensor(bag_label), torch.stack(img_labels_8class), name_tensor
    ...
    def random_choose_nocancer_img(self):
        ...
        # 正样本数量
        print('pos num: ', len(self.bags_p[:]), 'neg num: ', len(self.bags_n[:]))
...
class PolypDataset_test(data.Dataset):
    def __init__(self, index_root, transform_list, is_transform, is_train=True, testsize=352):
        file_index, labels, doc_labels = read_paths_and_labels_with_doc_label(index_root)
    ...
    def __getitem__(self, index):
        ...
        return torch.stack(images_tensor), torch.tensor(label_tensor), name_tensor, torch.stack(img_labels_8class)
...
class PolypDataset_test_junior_senior(data.Dataset):
    def __init__(self, index_root, transform_list, is_transform, is_train=True, testsize=352):
        file_index, labels, doc_labels = read_paths_and_labels_with_doc_label(index_root)
        # problem_index = read_index_from_file("/nas/qingcheng.xjw/workspace/MIL-HP/configs/new_data_cross_fold/problem_patient.txt")
        self.is_train = is_train
        ...
        for i, folder_path in enumerate(file_index):
            ...
            if label == 1:
                ...
                self.names_n.append(folder)
                self.labels_n.append(0)
            self.doc_labels.append(doc_labels[i])
        if is_transform:
            self.transform = self.get_transform(transform_list)
        else:
            self.transform = transforms.Compose([
    ...
    def __getitem__(self, index):
        ...
        return torch.stack(images_tensor), torch.tensor(label_tensor), name_tensor, center, doc_label
