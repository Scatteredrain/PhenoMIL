...
class TransMIL(nn.Module):
    def __init__(self,embed_dim,dropout,act):
        ...
        self._fc1 = [nn.Linear(embed_dim, 512)]
    ...
    def forward(self, x):
        ...
        add_length = _H * _W - H
        h = torch.cat([h, h[:,:add_length,:].contiguous()],dim = 1) #[B, N, 512]

        #---->cls_token
        B = h.shape[0]
        cls_tokens = self.cls_token.expand(B, -1, -1).to(h.device).contiguous()
        h = torch.cat((cls_tokens, h), dim=1).contiguous()
