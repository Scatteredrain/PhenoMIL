...
from lib.pvtv2_mil import SupConPVT, pvt_v2_b2, SupConPVT_MultiLabel, SupConPVT_CL, SupConPVT_addSeg
...
def create_pvt_encoder(args, pretrained=False, only_CL=False):
    if args.cls_task == 'multi_cls':
        ...
        if only_CL:
            ...
            model = SupConPVT_addSeg(args=args, head='mlp', feat_dim=args.low_dim, num_class=args.num_class)
...
class Mean(nn.Module):
    def __init__(self, dim=1, keepdim=False):
        super().__init__()
        self.dim = dim          # 指定聚合的维度
        self.keepdim = keepdim  # 是否保留聚合后的维度
...
class PiCO(nn.Module):
    ...
    def __init__(self, args, base_encoder):
        ...
        if self.cls_task == 'multi_cls':
            ...
            if self.stable_queue:
                ...
                self.queue_assis = StableDynamicQueue(queue_size=args.moco_queue, buffer_size=args.batch_size, num_classes=args.num_class)
        ...
        if args.add_bag_head:
            ...
            self.bag_predict_head = Bag_aggregate_head(out_dim=2, feature_dim=512, hidden_dim=512)
        else:
    ...
    def forward(self, img_q, im_k=None, partial_Y=None, args=None, eval_only=False, start_upd_prot=False, semi_stage2=False, task='img', eval_all=False):
        ...
        if eval_all:
            logits_clsHead, q, feat_encoder = self.encoder_q(img_q, task='all')
        ...
        if eval_only and task == 'img':
            ...
            logits_clsHead, q, feat_encoder = self.encoder_q(img_q, task)
            score_clsHead = torch.sigmoid(logits_clsHead)
            # compute protoypical logits
            prototypes = self.prototypes.clone().detach() #[C+1,D]
            prototype_normal = prototypes[0].unsqueeze(0) # [1,D]
        ...
        if task == 'bag' and self.add_bag_head:
            # time_s = time.time()
            with torch.no_grad():
                logits_clsHead, q, feat_encoder = self.encoder_q(img_q, task='img')
                score_clsHead = torch.sigmoid(logits_clsHead)
        ...
        # time1 = time.time()
        output, q, feat_encoder = self.encoder_q(img_q, task)
        ...
        if semi_stage2:
            # print('predicted_scores', predicted_scores[:4])
            # indices_confident_img = (predicted_scores > self.conf_thres) | (predicted_scores < (1 - self.conf_thres))
            # indices_confident_img = (predicted_scores > torch.clamp(args.pos_conf_thres / 0.5 * self.thre_vec_cls, min=0.01, max=0.99).to(predicted_scores.device)) | \
            #     (predicted_scores < torch.clamp(args.neg_conf_thres / 0.5 * (1-self.thre_vec_cls), min=0.01, max=0.99).to(predicted_scores.device)) 
            indices_confident_img = get_confident_indices(predicted_scores, self.thre_vec_cls.to(predicted_scores.device), mask_labeled=mask, pos_conf_per=0.95, neg_conf_per=0.95) #[B,C]
            indices_confident_sample = (indices_confident_img.sum(1) == indices_confident_img.shape[1]) #[B]
            # uncertainty = 1 - (predicted_scores - self.thre_vec_cls).abs() * 2  
            # weights = torch.exp(-uncertainty / 0.5) # [0,1], 越不确定值越大
            # print('percent of confident samples:', indices_confident_sample.sum() / indices_confident_sample.shape[0])
            # q, pseudo_labels_b = q[indices_confident_sample], pseudo_labels_b[indices_confident_sample]
        else:   
            indices_confident_img = None
        ...
        if not semi_stage2:
            ...
            q_concat = concat_all_gather(q)
            pseudo_labels_b_concat = concat_all_gather(pseudo_labels_b)
            indices_confident_img_concat = concat_all_gather(indices_confident_img)
            self._update_prototypes(q_concat, pseudo_labels_b_concat, args, mask_conf=indices_confident_img_concat)
        ...
        with torch.no_grad():  # no gradient 
            ...
            _, k, _ = self.encoder_k(im_k, task)
            # undo shuffle
            k = self._batch_unshuffle_ddp(k, idx_unshuffle)

        if start_upd_prot and semi_stage2: # filter out unconfident labels when using SupCL
        ...
        logits_one_bag = self.bag_predict_head.fc(feat_encoder)

        return output, features, pseudo_labels, score_prot_b, indices_confident_img, logits_one_bag
