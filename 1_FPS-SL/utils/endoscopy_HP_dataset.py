...
class BalancedEpochSampler(DistributedSampler):
    ...
    def __init__(self, dataset, num_replicas=None, rank=None, shuffle=True, seed=0):
        ...
        self.num_unlabeled = len(self.unlabeled_idx)
        
        # 验证数据集有效性
        if self.num_labeled == 0:
            raise ValueError("all unlabeled !")
        if len(self.unlabeled_idx) == 0:
            raise ValueError("all labeled !")
        
        self.epoch = 0
        self.total_size = 2 * self.num_labeled if self.num_labeled < self.num_unlabeled else self.num_unlabeled*2 # 每个epoch总样本量
    ...
    def __iter__(self):
        ...
        if self.num_labeled < self.num_unlabeled:
            ...
            selected_labeled = random.sample(self.labeled_idx, self.num_unlabeled)
            # 合并索引并打乱
            combined = self.unlabeled_idx + selected_labeled
...
def load_img_dataset(args, batch_size, debug=False, transform_path='./configs/transform.yaml'):
    ...
    train_dataset = PolypDataset_instances(
                    ...
                    return_bag= False, is_train=True, is_semi=True, only_labeled=True, cls_task=args.cls_task,args=args,
    ...
    eval_loader_img = torch.utils.data.DataLoader(dataset=eval_dataset_img,
                                    ...
                                    # shuffle = False,
                                    shuffle=False,
                                    sampler =torch.utils.data.distributed.DistributedSampler(eval_dataset_img, shuffle=False),
                                    num_workers=8,
                                    pin_memory=True,
                                    # prefetch_factor=2, 
                                    drop_last=False)
...
def load_semi_img_dataset(args, batch_size, debug=False, transform_path='./configs/transform.yaml'):
    print('transforms path:', transform_path)
    transform_list = load_config(transform_path)
    transform_list_weak, transform_list_strong, transform_list_test = transform_list['weak'], transform_list['strong'], transform_list['test']
    ## train with instances
    print('\n#### load the semi-train dateset ####')
    train_dataset = PolypDataset_instances(
                    num_class = args.num_class,
                    index_root = args.train_file,
                    transform_list_weak=transform_list_weak,
                    ...
                    transform_list_test=transform_list_test,
                    is_transform=True, size=transform_list_weak['resize']['size'][0],debug=debug,
                    return_bag= False, is_train=True, is_semi=True, only_labeled=False, cls_task=args.cls_task,
                    no_both=False, no_neither=True, constrain_no_either_nums=args.train_constrain_no_neither_nums*2,args=args
                    )#no_both=args.train_no_both, no_neither=args.train_no_neither)
    # train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset, shuffle=True)
    train_sampler = BalancedEpochSampler(train_dataset, num_replicas=args.world_size, rank=dist.get_rank(), shuffle=True) 
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset,
                                    batch_size=batch_size,
                                    shuffle=train_sampler is None,
                                    sampler=train_sampler,
...
def load_bag_dataset(args, batch_size, debug=False, transform_path='./configs/transform.yaml'):
    transform_list = load_config(transform_path)
    transform_list_weak, transform_list_strong, transform_list_test = transform_list['weak'], transform_list['strong'], transform_list['test']
    ...
    train_dataset_bag = PolypDataset_bagsv2(
                    ...
                    return_bag=True, is_train=True, bag_size=-1)
    ...
    eval_dataset_bag = PolypDataset_bagsv2(
                    ...
                    transform_list_strong=transform_list_strong,
                    transform_list_test=transform_list_test,
                    size=transform_list_weak['resize']['size'][0],debug=debug,
                    no_both=False, no_neither=False,
                    return_bag=True, is_train=False, bag_size=-1)
...
class PolypDataset_bagsv2(Dataset):
    def __init__(self, index_root, transform_list_weak, transform_list_strong, transform_list_test, is_transform=False, is_train=True, size=352, 
        ...
        for i, folder_path in enumerate(file_index):
            ...
            if len(bag) == 0:
                continue
            self.bag_labels_ori.append(labels[i])
    ...
    def __getitem__(self, index):
        out = {}
        paths_one_bag = self.bags[index]
        labels_image_one_bag = self.bag_labels_img_8class[index]
        labels_bag = self.bag_labels[index]
        ...
        # print('len', len(bag_images))
        out['bag'] = bag_images
        out['label_bag'] = labels_bag
        out['labels_instance'] = labels_image_one_bag
        out['image_path'] = paths_one_bag
    ...
    def balance_label_sample(self, balance_type=0):
        ...
        print('ori pos & neg data num: ', len(positive_idxs[0]), len(negative_idxs[0]))
        if balance_type == 0:
        ...
        print('balanced pos & neg data num: ', self.positive_num, self.negative_num)
        # combined = list(zip(self.bags, self.labels, self.names))
        # random.shuffle(combined)
        # self.bags[:], self.labels[:], self.names[:] = zip(*combined)

class PolypDataset_instances(Dataset):
    def __init__(self, index_root, transform_list_weak, transform_list_strong, transform_list_test, is_transform=False, is_train=True, is_semi=False, size=352, 
                    debug=False, return_bag=False, only_labeled=False, num_class=2, 
                    eight_class=True, cls_task='multi_cls', no_both=False, no_neither=True, 
                    constrain_no_either_nums=0, add_extra_data=False, args=None,
                    only_unlabeled=False, unlabeled_pseudo_labels=None, mask_unlabeled_valid=None):
        file_index, labels = read_paths_and_labels(index_root)
        self.is_train = is_train
        self.num_class = num_class
        ...
        fuji_index = []
        if debug:
            file_index = file_index[:50] + file_index[-10:]
            labels = labels[:50] + labels[-10:]
        ...
        self.image_labels_8class = np.array(self.image_labels_8class)
        self.image_paths, self.image_corresponding_patient_indexs, self.image_corresponding_patient_labels = \
            np.array(self.image_paths), np.array(self.image_corresponding_patient_indexs), np.array(self.image_corresponding_patient_labels)

        # control the 'labeled' background part which is unprecise and too much
        background_labeled_indices = np.where(self.image_labels == -3)[0]
        if len(background_labeled_indices) > 0:
            np.random.seed(123)
            np.random.shuffle(background_labeled_indices)
            masked_labeled_bg_indices = background_labeled_indices[constrain_no_either_nums:]
            self.image_labels[masked_labeled_bg_indices] = -1
            self.image_labels_8class[masked_labeled_bg_indices] = [0.5]*8

        # control the labeled part
        if is_semi:
            self._mask_labeled_samples(p=args.mask_labeled_percent)

        # filtered
        if self.only_labeled:
            labeled_indices = np.where(self.image_labels != -1)[0]
            self.image_paths, self.image_corresponding_patient_indexs, self.image_corresponding_patient_labels = \
                self.image_paths[labeled_indices], self.image_corresponding_patient_indexs[labeled_indices], self.image_corresponding_patient_labels[labeled_indices]
            self.image_labels, self.image_labels_8class = self.image_labels[labeled_indices], self.image_labels_8class[labeled_indices]
        elif self.only_unlabeled:
            unlabeled_indices = np.where(self.image_labels == -1)[0]
            self.image_paths, self.image_corresponding_patient_indexs, self.image_corresponding_patient_labels = \
                self.image_paths[unlabeled_indices], self.image_corresponding_patient_indexs[unlabeled_indices], self.image_corresponding_patient_labels[unlabeled_indices]
            self.image_labels, self.image_labels_8class = self.image_labels[unlabeled_indices], self.image_labels_8class[unlabeled_indices]
    ...
    def __getitem__(self, index):
        ...
        img_corresponding_patient_index = self.image_corresponding_patient_indexs[index]
        out['label'], out['label_true'], out['label_corresponding_patient'], out['label_true_8class'], out['label_corresponding_patient_index'] = \
            img_label, img_label_true, img_corresponding_patient_label, img_label_true_8class, img_corresponding_patient_index
    ...
    def _mask_labeled_samples(self, p=0., seed=123):

        np.random.seed(seed)  
        
        labeled_mask = self.image_labels != -1
        labeled_indices = np.where(labeled_mask)[0]
        
        if len(labeled_indices) == 0:
            return
        
        num_mask = int(len(labeled_indices) * p)
        if num_mask <= 0:
            return
        
        mask_indices = np.random.choice(labeled_indices, size=num_mask, replace=False)
        
        self.image_labels[mask_indices] = -1
        self.image_labels_8class[mask_indices] = [0.5]*8  

        print('labeled data percent from {:.2f} to {:.2f}'.format(len(labeled_indices)/len(self.image_labels), len(np.where(self.image_labels != -1)[0])/len(self.image_labels)))
        return 
...
def generate_Class_Aware_thresholds(args, batch_size, model, pos_label_freq, debug=False, transform_path='./configs/transform_new.yaml'):
    ...
    train_dataset = PolypDataset_instances(
                    ...
                    constrain_no_either_nums=args.train_constrain_no_neither_nums,args=args,
    ...
    with torch.no_grad():
        for i, pack in enumerate(train_loader):
            ...
            score_clsHead, _, _, score_prot = model(X, args=args, eval_only=True)
